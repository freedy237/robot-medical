...
import { Users, Activity, Heart, Thermometer, AlertTriangle, RefreshCw, Wind } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case 'critique':
        return { class: 'bg-red-100 text-red-800', text: '⚠️ Critique' };
      case 'alerte':
        return { class: 'bg-yellow-100 text-yellow-800', text: '⚠️ Alerte' };
      case 'normal':
        return { class: 'bg-green-100 text-green-800', text: '✓ Normal' };
      default:
        return { class: 'bg-gray-100 text-gray-800', text: 'Non mesuré' };
    }
  };
...
<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
  {statsConfig.map((stat, index) => {
    ...
    return (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Évolution des Paramètres (7 jours)</h3>
          <DynamicLineChart />
        </div>
        <div className="card p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Répartition des Patients</h3>
          <DynamicBarChart />
        </div>
      </div>
      ...
      <div className="card p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">
          Dernières Mesures du Jour ({recentMeasures.length})
          {isRefreshing && (
        ...
        {recentMeasures.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <div>Aucune mesure enregistrée aujourd'hui</div>
            <div className="text-sm mt-1">Les nouvelles mesures apparaîtront automatiquement</div>
            <button 
              onClick={handleManualRefresh}
