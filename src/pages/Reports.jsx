...
const Reports = () => {
  ...
  const reportTemplates = [
    { 
      ...
      description: 'Bilan complet des mesures de la semaine', 
      icon: FileText,
      type: 'weekly'
    },
    { 
      name: 'Analyse des Tendances', 
      description: 'Évolution des paramètres sur 30 jours', 
      icon: BarChart3,
      type: 'trends'
    },
    { 
      name: 'Rapport des Alertes', 
      description: 'Synthèse des alertes et incidents', 
      icon: AlertTriangle,
      type: 'alerts'
    },
  ];
  ...
  const generateReport = async (reportType, format = 'pdf') => {
    ...
    try {
      ...
      // Message de succès
      alert(`Rapport "${getReportTitle(reportType)}" généré avec succès!`);
  ...
  const sendReport = (reportType) => {
    const email = window.prompt('Entrez l\'adresse email pour envoyer le rapport:');
    if (email) {
      // Simulation d'envoi
      setTimeout(() => {
        alert(`Rapport "${getReportTitle(reportType)}" envoyé à ${email}`);
      }, 1000);
    }
  };
  ...
  const handleGenerateWithFormat = (reportType) => {
    const format = window.prompt(
      `Choisissez le format pour "${getReportTitle(reportType)}":\n1 - PDF\n2 - CSV\n3 - Excel\n4 - JSON\n\nEntrez le numéro:`
    );
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-gray-800">Rapports et Analytics</h1>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <h3 className="text-sm font-semibold text-gray-800 mb-3">Formats d'exportation rapide</h3>
      ...
      <div className="card p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Répartition par Tranche d'Âge</h3>
        <DynamicBarChart />
      </div>

      {/* Rapports récents */}
      <div className="card p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Rapports Générés Récemment</h3>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Généré le {report.date.toLocaleDateString('fr-FR')} • {report.size}
                  </p>
                </div>
              </div>
              <button 
